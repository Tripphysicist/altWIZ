function obs = getAltimeterObsWW3(loadSatList, mdLat, mdLon, mdLonO, altPath, crossesPrime)
...
for i = 1:length(loadSatList)
    ...
    for j = 1:length(altFileList)
        ...
        if crossesPrime
            ...
            if altNorth + 1 >= mdLat(1) & altNorth - 1 <= mdLat(end) & ...
                    altEast + 1 >= mdLonO(1) & altEast - 1 <= mdLonO(end)
            ...
            if altNorth + 1 >= mdLat(1) & altNorth - 1 <= mdLat(end) & ...
                    altEast + 1 >= mdLon(1) & altEast - 1 <= mdLon(end)
                %         if altNorth >= 40 & altNorth <= 41 & ...
                %                 altEast >= 200 & altEast <= 201
                fileName = [altFilePath altFileList(j).name];
                %            altInfo = ncinfo(fileName);
